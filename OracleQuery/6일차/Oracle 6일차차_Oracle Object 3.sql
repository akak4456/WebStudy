...
AS
SELECT E.EMP_NO, E.EMP_NAME, J.JOB_NAME, D.DEPT_TITLE, L.LOCAL_NAME
FROM EMPLOYEE E
...
AS
SELECT E.EMP_ID, E.EMP_NAME, J.JOB_NAME, NVL(D.DEPT_TITLE, '미정'), NVL((SELECT EMP_NAME FROM EMPLOYEE WHERE EMP_ID = E.MANAGER_ID),'미정'), E.HIRE_DATE
FROM EMPLOYEE E
...
-- 작업을 할때 데이터베읏 서버(오라클)에 의해 자동으로 갱신되는 테이블임.
...
-- 2.2.5 NOCYCLE : 시퀀스 값이 최대값까지 증가를 완료하면 CYCLE은 START WITH로 다시 시작하라.
-- 2.2.6 NOCACHE : 메모리 상에서 시퀀스값을 관리, 기본값 20, NOCACCHE를 안하면 갑자기 시퀀스값이 증가하게 됨
...
-- 2.3.2 사용법
-- 시퀀스명.NEXTVAL 또는 시퀀스명.CURRVAL 을
...
INSERT INTO ORDER_TBL (ORDER_NO,USER_ID, PRODUCT_ID, PRODUCT_CNT)
VALUES (SEQ_ORDER_NO.NEXTVAL, 'kang', 'saewookkang', 5);
-- * gam님이 gamjakkang상품을 30개 주문하셨습니다.
INSERT INTO ORDER_TBL (ORDER_NO,USER_ID, PRODUCT_ID, PRODUCT_CNT)
VALUES (SEQ_ORDER_NO.NEXTVAL, 'gam', 'gamjakkang', 30);
-- * ring님이 onionring상품을 50개 주문하셨습니다.
INSERT INTO ORDER_TBL (ORDER_NO,USER_ID, PRODUCT_ID, PRODUCT_CNT)
VALUES (SEQ_ORDER_NO.NEXTVAL, 'ring', 'onionring', 50);
