...
FOR EACH ROW
BEGIN
    IF(:OLD.SALARY != :NEW.SALARY) -- EMPLOYEE 테이블에 UPDATE가 되면 무조건 실행되는 것을 방지
    THEN
...
-- 의사레코드 OLD, NEW
-- FOR EACH ROW를 사용
-- 1. INSERT : OLD -> NULL, NEW (OLD가 없다)
-- 2. UPDATE : OLD, NEW
-- 3. DELETE : OLD, NEW -> NULL (NEW가 없다)

-- Before Trigger: As per the name, these triggers are fired prior to creating the row in the table.
-- Subsequently, since the row has not yet been created you have full access to the :new.table_element field. 
-- This allows for data cleansing and uniformity if unwanted/malformed data is attempting to be inserted/updated. 
-- This is just a basic example, but you need to utilize the before trigger any time you may require access to the ":new" data.
-- After Trigger: Since the after trigger fires once the row has already been created, 
-- these triggers are typically utilized when you want logic to occur due to the row.
-- For example, if you have an address table and a user updates his/her address, 
-- then you may want to update the address reference ids in an xref table upon creation 
-- (if you happen to be retaining all old addresses as well). Also, unlike the before trigger, 
-- you do not have access to modify any of the column values since the row already exists in the table.
...
(
    ...
    -- CONSTRAINT FK_PRODUCTID REFERENCES PRODUCT(PCODE),
);
