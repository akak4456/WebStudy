...
*/      
-- 1.1.3 날짜 처리 함수
--      - ADD_MONTHS, MONTHES_BETWEEN, LAST_DAY, EXTRACT, SYSDATE
-- @실습문제
/*
...
--	ex) 선동일	    1962	    0.3
--	ex) 송은희	    1963  	    0
SELECT EMP_NAME "직원명", '19'||SUBSTR(EMP_NO,1,2) "년생", NVL(BONUS, 0) "보너스" FROM EMPLOYEE WHERE SUBSTR(EMP_NO,1,2) BETWEEN '60' AND '69';
...
--	ex) 홍길동 771120-1******
SELECT EMP_NAME "이름", RPAD(SUBSTR(EMP_NO,1,8),LENGTH(EMP_NO),'*') "주민번호" FROM EMPLOYEE;
...
--	* 주말도 포함 , 소수점 아래는 버림
SELECT EMP_NAME "직원명",  HIRE_DATE "입사일", FLOOR(SYSDATE - HIRE_DATE) "오늘까지의 근무일수" FROM EMPLOYEE;
...
SELECT EMP_NAME "직원명", DEPT_CODE "직급코드", TO_CHAR(FLOOR(SALARY * 12 * (1 + NVL(BONUS, 0))), 'L999,999,999') "연봉(원)"
FROM EMPLOYEE;
--11. 사원명과, 부서명을 출력하세요.
...
-- DECODE 함수
-- 사용법 : DECODE(변하는 값, 조건값, 참일때 값, 조건값, 참일때 값, 모두 아닐 때 값)
-- 예시 : DECODE(변하는 값, 1, '빨강', 2, '노랑', '파랑)
-- CASE 함수
-- 사용법 : CASE WHEN 변하는값 = 조건값 THEN 참일 때 값 WHEN 변하는값 = 조건값 THEN 참일때 값 ELSE 모두 아닐때 값 END
...
SELECT JOB_CODE "직급" , COUNT(*) "직급별 사원수", FLOOR(AVG(SALARY)) "평균급여" FROM EMPLOYEE
WHERE JOB_CODE != 'J1'
GROUP BY JOB_CODE
...
SELECT DECODE(SUBSTR(EMP_NO,8,1), '1','남','2','여','3','남','4','여') "성별", FLOOR(AVG(SALARY)) "급여의 평균", SUM(SALARY) "급여의 합계", COUNT(SALARY) "인원수"
FROM EMPLOYEE
...
-- 실습7
-- 부서내 성별 인원수를 구하시오
SELECT DEPT_CODE, DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여') "성별", COUNT(*)
FROM EMPLOYEE
...
-- HAVING 실습문제
-- 실습문제1
-- 부서별 인원수가 5보다 많은 부서와 인원수를 출력하세요.
...
-- 실습문제3
-- 매니저가 관리하는 사원이 2명이상인 매니저아이디와 관리하는 사원 수를 출력하세요.
SELECT MANAGER_ID "매니저아이디", COUNT(*) "관리하는 사원 수"
FROM EMPLOYEE
GROUP BY MANAGER_ID
HAVING COUNT(*) >= 2;
