...
SELECT J.JOB_NAME "직급명", E.EMP_NAME "사원명"
FROM EMPLOYEE E
JOIN JOB J
ON J.JOB_CODE = E.JOB_CODE AND E.SAL_LEVEL IN (SELECT SAL_LEVEL FROM EMPLOYEE WHERE EMP_NAME IN ('차태연', '전지연'));
-- @실습문제2
-- Asia1지역에 근무하는 직원의 정보(부서코드, 사원명)를 출력하세요.
SELECT DEPT_ID FROM DEPARTMENT JOIN LOCATION ON LOCATION_ID = LOCAL_CODE AND LOCAL_NAME=UPPER('Asia1');
SELECT DEPT_CODE, EMP_NAME
FROM EMPLOYEE
WHERE DEPT_CODE IN (SELECT DEPT_ID FROM DEPARTMENT JOIN LOCATION ON LOCATION_ID = LOCAL_CODE AND LOCAL_NAME=UPPER('Asia1'));
...
SELECT DEPT_CODE, EMP_NAME, (SELECT FLOOR(AVG(SALARY)) FROM EMPLOYEE GROUP BY DEPT_CODE HAVING DEPT_CODE = E.DEPT_CODE) "월평균급여"
FROM EMPLOYEE E
WHERE EXISTS(SELECT 1 FROM EMPLOYEE WHERE E.DEPT_CODE = DEPT_CODE AND EMP_NAME = '심봉선');
...
-- 2. 월평균급여 출력
-- - 메인쿼리에 월평균급여 추가
SELECT DEPT_CODE, EMP_NAME, (SELECT FLOOR(AVG(SALARY)) FROM EMPLOYEE WHERE DEPT_CODE = E.DEPT_CODE) "월평균급여"
FROM EMPLOYEE E WHERE EXISTS (SELECT 1 FROM EMPLOYEE WHERE DEPT_CODE = E.DEPT_CODE AND EMP_NAME ='심봉선');
-- @실습문제4
-- 직급이 J1, J2, J3이 아닌 사원중에서 자신의 부서별 평균급여보다 많은 급여를 받는
-- 직원의 부서코드, 사원명, 급여, (부서별 급여평균) 정보를 출력하시오.
SELECT * FROM EMPLOYEE;
SELECT DEPT_CODE "부서 코드", EMP_NAME "사원명", SALARY "급여", JOB_CODE, (SELECT FLOOR(AVG(SALARY)) FROM EMPLOYEE WHERE DEPT_CODE = E.DEPT_CODE) "부서별 급여 평균"
FROM EMPLOYEE E
